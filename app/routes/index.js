...
export default Route.extend({
  ...
  fillNowPieData: function() {
    ...
    const promises = cities.map(city => {
      return $.getJSON(
        'https://stats.waze.su/data.php?a=city&type=1&format=json&id=' + city.id
      ).then(data => {
        return {
          id: city.id,
    ...
    all(promises).then(items => {
      items.forEach(data => {
        if (data.online > 0) {
          this.get('controller.nowData').pushObject(data);
        }
      });
  ...
  fillMaxPieData: function() {
    ...
    const promises = cities.map(city => {
      return $.getJSON(
        'https://stats.waze.su/data.php?a=city&type=365&format=json&id=' +
          city.id
      ).then(data => {
    ...
    all(promises).then(items => {
      items.forEach(data => {
        if (data.online > 0) {
          this.get('controller.maxData').pushObject(data);
        }
      });
